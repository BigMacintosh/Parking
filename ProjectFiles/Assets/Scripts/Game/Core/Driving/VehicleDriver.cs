...
//        }
        ...
        public void ApplyInputs(VehicleInputState inputs) {
            var grounded = false;
            turn = maxSteer * inputs.Turn;

            foreach (var wheel in driveWheels) {
                grounded                                 = wheel.CheckGround() || grounded;
                wheel.gameObject.transform.localRotation = Quaternion.Euler(0, turn, 0);
            }

            //Driving Forces
            if (acceptinput && inputs.Drive != 0 && grounded && Mathf.Abs(GetForward()) < maxSpeed) {
                body.AddForce(body.mass * accel * inputs.Drive * transform.forward);
            }

            //Turning Forces
            if (inputs.Turn != 0 && grounded && body.angularVelocity.magnitude < maxSteer / 30f &&
                Mathf.Abs(GetForward())     > 0.5f) {
                body.AddTorque(turn * body.mass * 2f * (GetForward() / Mathf.Abs(GetForward())) * transform.up);
            }

            if (acceptinput && inputs.Jump != 0 && grounded) {
                body.AddForce(body.mass * accel * inputs.Jump * transform.up * 10f);
            }
